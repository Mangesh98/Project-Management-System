...
const CreateProjectForm = () => {
    ...
    function handleTagsChange(item: string): void {
        ...
        const updatedTags = tags.includes(item) ? tags.filter(tag => tag !== item) : [...tags, item];
    ...
    return (
        <Form {...form}>
            <form className="space-y-3" onSubmit={form.handleSubmit(handleSubmit)}>
                    ...
                    render={({ field }) => <FormItem>
                        <FormControl >
                            ...
                            >
                                ...
                                <SelectContent>
                                    {tags.map(tag => (
                                        <SelectItem key={tag.value} value={tag.value}>{tag.name}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </FormControl>
                ...
                <DialogClose>
                    {false ? <div><p>You can create only 3 project with free plan, please upgrade your paln</p></div> : <Button type="submit" className="w-full mt-5">Create Project</Button>}
                </DialogClose>
